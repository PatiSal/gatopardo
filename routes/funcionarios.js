...
router
    ...
    .get((req, res)=>{
        var query = "SELECT count(id_cliente) AS totalClientes FROM Cliente; SELECT count(id_artista) AS totalArtistas FROM Artista; SELECT count(id_evento) AS totalEventos FROM Evento;SELECT count(id_func) AS totalFuncionarios FROM Funcionario; SELECT count(id_venda) AS totalVendas FROM Venda;SELECT count(id_compra) AS totalCompras FROM Compra; SELECT vendas - compras as lucroTotal FROM (select sum(total_vendas) as vendas from (select titulo_produto, sum(unidades_vendidas * preco_venda) as total_vendas from Produto inner join Produto_Venda using(id_produto) group by titulo_produto) as subconsulta_a) as subsubconsulta_a, (select sum(total_compras) as compras from (select titulo_produto, sum(unidades_compradas * preco_compra) as total_compras from Produto inner join Produto_Compra using(id_produto) group by titulo_produto) as subconsulta_b) as subsubconsulta_b; SELECT round(avg(floor(datediff(now(),dn_cliente) / 365.25))) as idadeMedia from Cliente; SELECT morada_cliente from Cliente group by morada_cliente having count(morada_cliente) = ( select max(conta) from (select morada_cliente, count(morada_cliente) as conta from Cliente group by morada_cliente) as cons); SELECT genero_cliente as maisVendasGenero from Cliente group by genero_cliente having count(id_cliente) = (select max(conta) from (select genero_cliente, count(id_cliente) as conta from Cliente group by genero_cliente) as cons); SELECT titulo_produto as produtoMaisVendido from Produto inner join Produto_Venda using(id_produto) GROUP BY titulo_produto having sum(unidades_vendidas)  = (select max(conta) from (select titulo_produto, sum(unidades_vendidas) as conta from Produto_Venda inner join Produto using(id_produto) GROUP BY titulo_produto) as suconsulta);SELECT nome_func as melhorFuncionario from Funcionario inner join Venda using(id_func) group by nome_func having count(id_venda) = (select max(total_vendas) from (select nome_func, count(id_venda) as total_vendas from Funcionario inner join Venda using(id_func) group by nome_func) as subconsulta);";
        ...
        runQuery(query, function (err, result, fields) {
            ...
            console.log(result[0][0].totalClientes)
            console.log(result[1][0].totalArtistas)
            //TOTAL CLIENTES
            ...
            //TOTAL DE VENDAS
            html += "<div class='col card_dash'><div class='number'><p class='quantidade'>" + result[4][0].totalVendas + "</p></div><div class='titulo'><h2>Total de Vendas </h2></div></div>\n";
            // TOTAL DE COMPRAS
            html += "<div class='col card_dash'><div class='number'><p class='quantidade'>" + result[5][0].totalCompras + "</p></div><div class='titulo'><h2>Total de Compras </h2></div></div>\n";
            //LUCRO TOTAL
            html += "<div class='col card_dash'><div class='number'><p class='quantidade'>" + result[6][0].lucroTotal + "</p></div><div class='titulo'><h2>Lucro Total</h2></div></div>\n";
            ...
            html += "<div class='col card_dash'><div class='titulo_info'><h2>Produto mais Vendido</h2></div><div class='info_dash'><p class='quantidade'>" + result[10][0].produtoMaisVendido + "</p></div></div>\n";
            //MELHOR CLIENTE
            html += "<div class='col card_dash'><div class='titulo_info'><h2>Melhor funcionario</h2></div><div class='info_dash'><p class='quantidade'>" + result[11 ][0].melhorFuncionario + "</p></div></div>\n";
    ...
    .get((req, res)=>{
        ...
        var html="";
            ...
            for (var i = 0; i < result[0].length; i++) {
                html += "<tr>\n";
                html += "<th scope='row'>" + result[0][i].id_produto + "</th>\n";
                html += "<td><img width='25px' src='/uploads/" + result[0][i].fotografia_produto
                + "'></td>\n";
                html += "<td>" + result[0][i].titulo_produto  + "</td>\n";
                html += "<td>"  + new Date(result[0][i].dt_elaboracao_produto).getFullYear() + "/" + new Date(result[0][i].dt_elaboracao_produto).getMonth() + "/" + new Date(result[0][i].dt_elaboracao_produto).getDate() 
                 + "</td>\n";
        ...
        if(req.query.id_produto){
            ...
            runQuery(query, function(err, result, fields){
                if(result && result.length > 0) {    
                    ...
                    //DATA ELABORAÇÃO
                    html += "<div class='row mb-3 inputproduto'><label for='data' class='col-sm-6 col-form-label col-form-label-sm'>Data de Elaboração:</label><div class='col-sm-6'><input class='form-control form-control-sm' id='data' value='"+ new Date(result[0].dt_elaboracao_produto).getFullYear() + "/" + new Date(result[0].dt_elaboracao_produto).getMonth() + "/" + new Date(result[0].dt_elaboracao_produto).getDate()  +"' name='dt_elaboracao_produto'></div></div>\n";
                    //MATERIAL UTILIZADO
            ...
            res.send("produto não disponivel")
        }
    }); 
...
.post((req, res)=>{
    ...
    //Se não se adicionar nova fotografia 
        ...
        if(req.query.id_produto){
            ...
            runQuery(query, function(err, result, fields){
                if(result && result.length > 0) {    
                    ...
                    html += "<form name='formulario' method='post' action=''processa_elimina_produto' enctype='multipart/form-data' >\n";
                    html += "<div class='row'>\n";
                    html += "  <div class='col-4 imgaltera'>\n";
                    html += " <input type='file' id='file' name='fotografia_produto'>\n";
                    // FOTOGRAFIA inserida dinamicmente
...
//=======================================
    ...
    .route("/artistas")
    .get((req, res)=>{
        var query = " SELECT id_artista, localidade_artista, fotografia_artista, nome_artistico_artista, nome_real_artista, categoria_arte_artista, facebook_artista, instagram_artista, behance_artista, website_artista, biografia_artista, mail_artista, username_artista FROM Artista;";
        ...
        runQuery(query, function (err, result, fields) {
            ...
            for (var i = 0; i < result.length; i++) {
                html += "<tr>\n";
                html += "<th scope='row'>" + result[i].id_artista + "</th>\n";
...
.get((req, res)=>{
    ...
    runQuery(query, function (err, result, fields) {
        ...
        for (var i = 0; i < result.length; i++) {
            html += "<tr>\n";
            html += "<th scope='row'>" + result[i].id_evento + "</th>\n";
            html += "<td>" + result[i].titulo_evento + "</td>\n";
            html += "<td>" + new Date(result[i].dt_inicio_evento).getFullYear() + "/" + new Date(result[i].dt_inicio_evento).getMonth() + "/" + new Date(result[i].dt_inicio_evento).getDate() 
            + "</td>\n";
            html += "<td>" + new Date(result[i].dt_fim_evento).getFullYear() + "/" + new Date(result[i].dt_fim_evento).getMonth() + "/" + new Date(result[i].dt_fim_evento).getDate() 
            + "</td>\n";
...
//=======================================
    ...
    .route("/clientes")
    .get((req, res)=>{
        var query = " SELECT id_cliente, nome_cliente, username_cliente, genero_cliente, dn_cliente, morada_cliente, profissao_cliente, nif_cliente, mail_cliente FROM Cliente;";
        ...
        for (var i = 0; i < result.length; i++) {
            html += "<tr>\n";
            html += "<th scope='row'>" + result[i].id_cliente + "</th>\n";
            html += "<td>" + result[i].nome_cliente + "</td>\n";
            html += "<td>" + result[i].username_cliente + "</td>\n";
            html += "<td>" + result[i].genero_cliente + "</td>\n";
            html += "<td>" + new Date(result[i].dn_cliente).getFullYear() + "/" + new Date(result[i].dn_cliente).getMonth() + "/" + new Date(result[i].dn_cliente).getDate() 
            + "</td>\n";
...
//=======================================
    ...
    .route("/funcionarios")
    .get((req, res)=>{
        var query = "SELECT id_func, nome_func, username_func, dn_func, morada_func, mail_func, contacto_func FROM Funcionario;";
        ...
        runQuery(query, function(err, result, fields){
            ...
            for (var i = 0; i < result.length; i++) {
                html += "<tr>\n";
                html += "<th scope='row'>" + result[i].id_func + "</th>\n";
                html += "<td>" + result[i].nome_func + "</td>\n";
                html += "<td>" + result[i].username_func + "</td>\n";
                html += "<td>" + new Date(result[i].dn_func).getFullYear() + "/" + new Date(result[i].dn_func).getMonth() + "/" + new Date(result[i].dn_func).getDate() 
                + "</td>\n";
...
router
    ...
    if (req.query.id_func) {
        ...
        runQuery(query, function (err, result, fields) {
            ...
            if (result && result.affectedRows > 0) {
                //direcionar para a pagina de funcs
...
//=======================================
    ...
    .get((req, res)=>{
        ...
        runQuery(query, function (err, result, fields) {
            ...
            for (var i = 0; i < result.length; i++) {
                html += "<tr>\n";
                html += "<th scope='row'>" + result[i].id_compra + "</th>\n";
                html += "<td> " + result[i].titulo_produto + " </td>\n";
                html += "<td> " + result[i].unidades_compradas + " </td>\n";
                html += "<td> " + result[i].preco_compra + " €</td>\n";
                html += "<td> " + result[i].nome_artistico_artista + " </td>\n";
                html += "<td> " + result[i].nome_func + " </td>\n";
                html += "<td> " + new Date(result[i].dt_compra).getFullYear() + "/" + new Date(result[i].dt_compra).getMonth() + "/" + new Date(result[i].dt_compra).getDate() 
                + "</td>\n";
...
.get((req, res)=>{
        //verificar se os campos foram preenchidos
        if(req.query.unidades_compradas ){ 
        ...
        if(req.query.id_compra){
            ...
            runQuery(query, function(err, result, fields){
                if(result && result.length > 0) {    
                    ...
                    //DATA ELABORAÇÃO
                    html += "<div class='row mb-3 inputproduto'><label for='data' class='col-sm-2 col-form-label col-form-label-sm'>Data de Compra:</label><div class='col-sm-10'><input type='text' class='form-control form-control-sm' id='data' name='dt_compra' value='"  + new Date(result[0][0].dt_compra).getFullYear() + "/" + new Date(result[0][0].dt_compra).getMonth() + "/" + new Date(result[0][0].dt_compra).getDate() 
                    + "'></div></div>\n";
                    //HORA da CCOMPRA
...
.get((req, res)=>{
        ...
        if(req.query.unidades_compradas ){ 
         ...
         var query ="UPDATE Compra SET dt_compra = '" + req.query.dt_compra + "', hora_compra = '" + req.query.hora_compra + "', id_func = '" + req.query.id_func + "' WHERE id_compra ='" + req.query.id_compra + "'; UPDATE Produto_Compra SET unidades_compradas = '" + req.query.unidades_compradas + "' WHERE id_compra ='" + req.query.id_compra + "';";   
            ...
            runQuery(query, function (err, result, fields) {     
                //adicionar fotografia a pasta
              // if(result[1] && result.affectedRows[1] > 0){
                    //direcionar para a pagina de produtos
                    res.redirect('compras'); 
               // }
              /*  else{
                var html ='';
                html = query;
                html += "<p> O produto não foi inserido, por favor verifique os campos </p>"
                html += result[1];
...
//=======================================
    ...
    .get((req, res)=>{
        ...
        var html="";
            ...
            for (var i = 0; i < result.length; i++) {
                html += "<tr>\n";
                html += "<td>" + result[i].id_venda+ "</td>\n";
                html += "<td>" + result[i].titulo_produto + "</td>\n"; 
                html += "<td>" + result[i].unidades_vendidas + "</td>\n";
                html += "<td>" + result[i].preco_total + "€</td>\n";
                html += "<td>" + result[i].nome_cliente+ "</td>\n";
                html += "<td>" + result[i].nome_func + "</td>\n";
                html += "<td>"  + new Date(result[i].dt_venda).getFullYear() + "/" + new Date(result[i].dt_venda).getMonth() + "/" + new Date(result[i].dt_venda).getDate() 
                + "</td>\n";
...
.get((req, res)=>{
        //verificar se os campos foram preenchidos
        if(req.query.unidades_vendidas){ 
            ...
            runQuery(query, function (err, result, fields) {     
                    console.log(query)
               // if(result[0] && result.affectedRows[0] > 0){
                    //direcionar para a pagina de produtos
                    res.redirect('vendas'); 
               // }
              /*  else{
...
//=======================================
    ...
    .get((req, res)=>{
        ...
        runQuery(query, function (err, result, fields) {
            ...
            html += "<p>Data de nascimento: <span> "  + new Date(result[0].dn_func).getFullYear() + "/" + new Date(result[0].dn_func).getMonth() + "/" + new Date(result[0].dn_func).getDate() 
            + "</span></p>"
            ...
            html += " <div class='col-6 mudapass'>\n";
            html += "<p>Como sabe o site Gato Pardo está sob desenvolvimento, por enquanto a configuração de uma nova password está indisponível</p>"
...
.get((req, res)=>{
   ...
   // runQuery(query, function (err, result, fields) {
        ...
        //ANTIGA PASSWORD
        html += "<div class='col-6'><label>antiga password:</label><input  type='password' name='password_func' value=''></div>\n";

        //NOVA PASSWORD
        html += "<div class='col-6'><label>nova password:</label><input  type='password' name='nova_password_func'></div>\n";
...
.get((req, res)=>{
    ...
    runQuery(query, function (err, result, fields) {
        ...
        html += " <div class='input_func row'>\n";
            ...
            console.log( hash('req.query.password_func'))
           
            console.log("query password")
            console.log(result[0].password_func)
